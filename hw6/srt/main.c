...
/* Global variables to simplify multithreading */
int nthreads;
scene_t scene;
float image_pixels[width][height][3];

/* function used for multithreading: divides the calculation of
 * pixel values into chunks that can be parallelized */
void * generate_pixels(void * thread)
{
	...
	/* for every pixel where column%nthreads == thread */
	for (int px = *(int*)thread; px<width; px+=nthreads)
	{
		...
		for (int py = 0; py<height; ++py)
		{
			...
			*/
			...
			/* write the value to the image_pixels array */
			for (int i = 0; i < 3; i++)
			{
				scaled_color[i] = max(min(scaled_color[i], 255), 0);
				image_pixels[px][py][i] = scaled_color[i];
			}
		}
	}
	return NULL;
...
main( int argc, char **argv )
{
	...
	/* Create threads one at a time */
	for (i = 0; i < nthreads; i++)
	{
		thread_args[i] = i;
		result_code = pthread_create(&threads[i], NULL, generate_pixels, &thread_args[i]);
		if (result_code)
		{
			fprintf(stderr, "Error (%d): Failed to create thread", result_code);
			exit(-1);
		}
	}
	...
	for (i = 0; i < nthreads; i++)
	{
		...
		if (result_code)
		{
			fprintf(stderr, "Error (%d): Failed to join thread", result_code);
			exit(-1);
		}
	}

	/* Print out each pixel row by row */
	for (int i = 0; i < width; i++)
	{
		for (int j = 0; j < height; j++)
		{
			printf("%.0f %.0f %.0f\n", image_pixels[i][j][0], image_pixels[i][j][1], image_pixels[i][j][2]);
		}
		printf("\n");
	}
